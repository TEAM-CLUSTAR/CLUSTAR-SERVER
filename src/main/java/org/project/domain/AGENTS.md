...
- HTTP 요청·응답, 경로·상태 코드, 인증 인자 전달, 요청 검증을 기본 책임으로 둔다. 성공 응답은 `ResponseEntity<ApiResponse<T>>`를 기본 형식으로 사용한다.
- 요청 본문과 중첩 객체는 필요한 `@Valid`로 검증하며, 공개 API 문서화는 `@Operation`, `@Schema`를 기본 패턴으로 사용한다.
...
- 동시성, 외부 API 호출, 실패·재시도는 영향 범위와 현재 예외·이벤트 흐름을 검토한다. Controller 응답 조립과 Repository 쿼리 구현 세부사항은 Service 계약에 넣지 않는다.
- 외부 호출 결과를 저장해야 하면 호출 실패·타임아웃·부분 성공 시의 트랜잭션 경계를 명확히 한다. `InterruptedException` 처리 시 인터럽트 상태를 복원하고, 재시도는 일시 오류에 한정한다.

## Repository

- Spring Data JPA 인터페이스와 Custom/Impl QueryDSL 구현으로 영속성·조회만 담당한다. HTTP·인증 표현·비즈니스 정책을 넣지 않는다.
- 목록·연관 조회에서는 fetch join, 페이징, 컬렉션 조인으로 인한 N+1·중복·성능 영향을 검토한다. Memo의 ID 제한 뒤 fetch join 방식은 현재 성능 문제를 피하는 한 가지 기준이며, 다른 접근을 선택하면 성능·정합성 근거를 남긴다.
...
- 양방향 연관관계, `cascade`, `orphanRemoval`, fetch 전략, 논리 삭제 상태를 변경하면 데이터 무결성과 현재 조회 조건을 함께 검토한다. `ChatRoom`의 `@Where(is_deleted = false)`와 Memo의 `isDeleted` 조건은 현재 정책이므로, 변경 시 마이그레이션·조회 영향·대체 정책을 함께 제시한다.
- 스키마·컬럼·연관관계 변경은 마이그레이션과 운영 데이터 영향을 별도 검토한다.

## DTO

- `request`/`response` 및 필요한 레이어 경계 데이터만 표현한다. 요청 DTO에는 Bean Validation, 공개 API 필드에는 `@Schema`를 기본 문서화 방식으로 적용한다.
